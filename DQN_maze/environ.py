...
# we took 500 first (replaced all with 700)
class Environment:
    ...
    def __init__(self):
        ...
        # 500 was randomly chosen, change as you wish too! 
        self.image = np.zeros([int(500 * self.height), int(500 * self.width), 3],
                              dtype=np.uint8)
    ...
    def show(self, agent_state):
        ...
        for block in self.free_blocks:
            top_left = (int(500 * block[0][0]), int(500 * (1 - block[0][1])))
            bottom_right = (int(500 * block[1][0]), int(500 * (1 - block[1][1])))
            cv2.rectangle(self.image, top_left, bottom_right, (0, 0, 0), thickness=cv2.FILLED)

        # Draw the agent
        agent_centre = (int(agent_state[0] * 500), int((1 - agent_state[1]) * 500))
        agent_radius = int(0.01 * 500)
        agent_colour = (0, 0, 255)
        cv2.circle(self.image, agent_centre, agent_radius, agent_colour, cv2.FILLED)

        # Draw the goal
        goal_centre = (int(self.goal_state[0] * 500), int((1 - self.goal_state[1]) * 500))
        goal_radius = int(0.01 * 500)
        goal_colour = (0, 255, 0)
        cv2.circle(self.image, goal_centre, goal_radius, goal_colour, cv2.FILLED)

        # Show the image
        cv2.imshow("Environment_", self.image)
