...
def test_config(environment, config_path, flag_display=True, flag_plot=True):
    ...
    end_time = start_time + 800 #600,800,
    ...
    while time.time() < end_time:
        ...
        if agent.has_finished_episode():
            ...
            state = environment.init_state
            #agent.final_distance += (distance_to_goal)
            ...
            distance_trav.append(distance_to_goal)
    ...
    if flag_plot:
        ...
        plt.plot(x_axis, distance_trav)
        plt.ylabel('distance')
        plt.xlabel('number of episodes')
        plt.title('distance per episode')
        plt.savefig(config_path + "/distance_travelled.png")
        plt.close()
